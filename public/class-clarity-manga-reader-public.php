...
class Clarity_Manga_Reader_Public {
	...
	public function enqueue_scripts() {
		...
		if (strpos($url,'/read/') !== false) {
			...
			wp_enqueue_script( $this->plugin_name . "screenfull", plugin_dir_url( __FILE__ ) . 'js/screenfull.min.js');
		}
		//wp_enqueue_script( $this->plugin_name . "confetti", plugin_dir_url( __FILE__ ) . 'js/confetti.browser.min.js');
	...
	public function cmr_chaptersListShortcode($attributes){
		...
		if ($object) {
			...
			$contents = '<h1 class="manga-hebrew-title">' . get_post_meta($post->ID,'manga-hebrew-name',true) . '</h1>' . $atts['before'];
			$contents .= "<style>.post-title{display: none}</style>";
			foreach ( $object as $o ) {
				...
				if ( $has_images[0]->id ) {
					...
					if ( $current_volume != $previous_volume && $current_volume != 0 ) {
						//$vol = '<div class="volume-number">ווליום ' . $o->chapter_volume . '</div>';
					}
					$contents .= $vol;
	...
	public function cmr_read($atts){
		...
		$contents .= '<div id="manga-nav" class="manga-nav center">';
		$contents .= '<span style="display: inline" class="DesktopNav but1" ><a class="butsel" href="' . site_url() . '?p=' . $manga_id . '" data-mid="' . $manga_id . '" id="manga-title"><i style="
    padding: 5px;
" class="fas fa-book"></i>' . get_the_title( $manga_id ) . '</a></span>';
		//$contents .= '<div style="display: inline">';
		$contents .= '<select style="display: inline" class="butsel but2 DesktopNav" id="chapter-select"><i class="fas fa-stream"></i>';
		foreach ( $chapters as $chapter ) {
			...
			$contents .= '<option value="' . $chapter->chapter_number . '"' . $selected . "> צ'אפטר "  . $chapter->chapter_number . '</option>';
		}
		$contents .= '</select>';
		$contents .= '<select style="display: inline" class="butsel but3 DesktopNav" id="page-select">';
		for ( $i = 1; $i <= $total_ch_images; $i++ ) {
			...
			$contents .= '<option value="' . $i . '"' . $selected . '>עמוד ' . $i . '</option>';
		}
		$contents .= '</select>';
		//$contents .= '</div>';
		$contents .= '<span class="but4" style="display: inline">';
		//$contents .= '<a href="' . $previous_page . '" id="prev-page" data-ch="' . $previous_chapter . '" data-img="' . $previous_img . '"> <i class="fas fa-arrow-left"></i> </a>';
		//$contents .= '<a href="' . $next_page . '" id="next-page" data-ch="' . $next_chapter . '" data-img="' . $next_img . '"> <i class="fas fa-arrow-right"></i> </a>';
		$contents .= '<button class="butsel" id="screen-full" > <i class="fas fa-expand"></i> </button>';
		$contents .= '</span>';
		$contents .= '</div>';
		$contents .= '<div class="manga-image-link"><a href="' . $next_page . '" class="img-next-page" id="img-next-page">';
		$contents .= '<img src="' . $img_url . '" class="manga-image " style="max-height: auto;width: 95vw;text-align: center;margin: auto;">';
		$contents .= '</a></div>';
		$contents .= '</div>';

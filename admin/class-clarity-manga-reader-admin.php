...
*/
	...
	public function switchImages()
	{
		global $wpdb;
		$id1 = intval($_POST['src']);
		$id2 = intval($_POST['dest']);
		//$manage = new Manage();
		//$manage.switchImages($id1,$id2);
        
      	$imgname1 = $wpdb->get_var('SELECT image_name FROM ' . $wpdb->prefix . 'cmr_images WHERE id=' . $id1);;
        $imgname2 = $wpdb->get_var('SELECT image_name FROM ' . $wpdb->prefix . 'cmr_images WHERE id=' . $id2);;
		...
		echo json_encode(array("mes" => "Complete!"));
		exit(200);
		die();
		
	}

/*R_PATH . get_post($mid)->post_name );
	}
*/
	/*
	 *
	 ...
	 * 
	 * */
	public function post_saved($post)
	{
		if(isset($_POST['manga-hebrew-name']))
		{
			$name = sanitize_text_field($_POST['manga-hebrew-name']);
			update_post_meta($post,'manga-hebrew-name',$name);
		}
	}
	/**
	...
	function add_chapter_meta_box() {
		add_meta_box(
			'manga_hebrew_name',
			__( 'שם המנגה בעברית', 'manga_hebrew' ),
			array( $this, 'add_manga_hebrew_name_callback'),
			'manga'
		);
		add_meta_box(
			'manga_chapters',
	...
	function add_manga_hebrew_name_callback($post)
	{
		
		?>
		<div class="manga-name">
			<h3>
				:Enter The Name of The Manga in Hebrew
			</h3>
			<div class="input-box">
				
			
			<input name="manga-hebrew-name" value="<?php echo get_post_meta($post->ID,'manga-hebrew-name',true) ?>" id="manga-hebrew-name" type="type="text"">
</div>
</div>
		<?php
	...
	public function cmr_upload() {
		...
		if ($method == "POST") {
			...
			//order_images($_POST['chapter_id'], json_encode($result));
			// ---------
			$id = $_POST['chapter_id'];
			$pics = $wpdb->get_results( 'SELECT * FROM ' . $wpdb->prefix . 'cmr_images WHERE chapter_id=' . intval($id) );
			if(count($pics) > 0) {		
				$picsOrg = $pics;
			
				//die(json_encode($pics));
				/*usort($pics, function ($item1, $item2) {
    				return $item1->image_name <=> $item2->image_name;
				});*/
				usort($pics, function ($item1, $item2) {
    				return strnatcmp($item1->image_name, $item2->image_name) > 0;
				});
				for ($i = 0; $i < count($pics); $i++) {
					$wpdb->update($wpdb->prefix . 'cmr_images',array('image_name' => $pics[$i]->image_name), array("id"=>intval($picsOrg[$i]->id)));//
				}
				//die(json_encode($pics));
            }
			// ---------		
		...
		//echo $wpdb->get_results( 'SELECT * FROM ' . $wpdb->prefix . 'cmr_images WHERE chapter_id=' . $id, OBJECT );
